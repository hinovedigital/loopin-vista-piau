    ...
    <section className="section-light py-24">
      <div className="container mx-auto px-6 animate-on-scroll" ref={ref}>
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {testimonials.map((t) => (
            <div key={t.name} className="bg-background rounded-2xl p-8 border border-border shadow-sm space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-on-light font-body">{t.name}</p>
                  <p className="text-muted text-sm font-body">{t.biz}</p>
                </div>
              </div>
              <div className="text-primary text-sm">⭐⭐⭐⭐⭐</div>
              <p className="text-on-light/70 font-body leading-relaxed italic">
                "{renderQuote(t.quote)}"
              </p>
            </div>
          ))}
